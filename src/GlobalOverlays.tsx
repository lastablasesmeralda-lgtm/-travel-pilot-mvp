    ...
    return (
        <>
            ...
            <Modal visible={showSOSMenu} transparent animationType="fade">
                <View style={s.mf}>
                    <View style={[s.mc, { width: '90%' }]}>
                        ...
                        {[
                            { icon: '📞', title: 'LLAMAR AL HOTEL', sub: 'Notificar llegada tardía automáticamente', color: '#AF52DE', action: () => { setShowSOSMenu(false); Alert.alert('LLAMADA AL HOTEL', 'Travel-Pilot contactará al hotel vía Twilio para informar de tu llegada tardía.', [{ text: 'LLAMAR AHORA', onPress: async () => { speak('Iniciando sistema de llamadas. Contactando hotel.'); try { const res = await fetch(`http://192.168.1.128:3000/api/notifyHotel`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ hotelPhone: "+34600000000", passengerName: user?.email?.split('@')[0] || "Operativo", delayMinutes: apiPlan?.impact?.delayMinutes || 210 }) }); const data = await res.json(); if (res.ok) { Alert.alert('ÉXITO', 'El hotel ha sido notificado correctamente.'); } else { Alert.alert('AVISO', data.error || 'No se pudo contactar. ¿Están las API keys de Twilio?'); } } catch (e) { Alert.alert('ERROR', 'Fallo de conexión al llamar al hotel.'); } } }, { text: 'CANCELAR', style: 'cancel' }]); } },
                            ...
                            { icon: '🛡️', title: 'ESCUDO LEGAL EU261', sub: 'Reclamar compensación por retraso', color: '#27C93F', action: () => { setShowSOSMenu(false); setLegalShieldActive(true); setCompensationEligible(true); setTab('Vault'); speak('Escudo legal activado. Iniciando expediente de reclamación EU261.'); } },
                        ...
                        <TouchableOpacity onPress={() => setShowSOSMenu(false)} style={{ marginTop: 15 }}>
            ...
            <Modal visible={!!user && (!!selectedPlan || showSOS)} transparent animationType="fade">
                <View style={s.mf}><View style={s.mc}>
                    <Text style={s.mt}>GENERACIÓN DE ESCENARIOS (IA)</Text>
                    {isGenerating ? (
                        <View style={{ marginVertical: 30, alignItems: 'center' }}>
                            <ActivityIndicator size="large" color="#AF52DE" />
                            <Text style={{ color: '#AF52DE', marginTop: 15, fontWeight: 'bold', fontSize: 12 }}>
                                {loadingStep === 0 && "CONECTANDO CON NÚCLEO GEMINI..."}
                                {loadingStep === 1 && "ANALIZANDO IMPACTO HOTELERO..."}
                                {loadingStep === 2 && "CALCULANDO RUTAS TÁCTICAS..."}
                                {loadingStep === 3 && "SINCRONIZANDO CON VAULT..."}
                            </Text>
                        </View>
                    ) : (
                        <>
                            {apiPlan?.impact && (
                                <View style={{ backgroundColor: '#111', width: '100%', padding: 12, borderRadius: 10, marginBottom: 15, borderWidth: 1, borderColor: apiPlan.impact.potentialLoss > 0 ? '#FF3B30' : '#4CD964' }}>
                                    <Text style={{ color: '#FFF', fontSize: 11, fontWeight: 'bold' }}>🧠 TRAVEL_IMPACT_REPORT:</Text>
                                    <Text style={{ color: '#FF3B30', fontSize: 10, marginTop: 4 }}>• {apiPlan.impact.hotelAlert}</Text>
                                </View>
                            )}
                            <Text style={{ color: '#666', fontSize: 10, textAlign: 'center', marginBottom: 10 }}>EL MOTOR DE RAZONAMIENTO HA CREADO 3 RUTAS:</Text>
                            <View style={{ width: '100%', marginTop: 20 }}>
                                {(apiPlan?.options || []).map((opt: any, idx: number) => (
                                    <TouchableOpacity
                                        ...
                                        onPress={() => {
                                            const msg = `${opt.title}: ${opt.description} (Costo: $${opt.estimatedCost})`;
                                            speak(msg);
                                ...
                                <TouchableOpacity onPress={() => { stopSpeak(); setShowSOS(false); setSelectedPlan(null); }} style={{ marginTop: 20, alignItems: 'center' }}><Text style={{ color: '#999' }}>CANCELAR / CERRAR</Text></TouchableOpacity>
                            </View>
                        </>
                    )}
                </View></View>
            ...
            < Modal visible={showBrowser} transparent animationType="fade" >
                <View style={s.mf}>
                    <View style={[s.mc, { width: '90%', height: '70%', padding: 0, overflow: 'hidden' }]}>
