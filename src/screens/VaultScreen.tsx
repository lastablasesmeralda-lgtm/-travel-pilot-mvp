    ...
    const confirmAndUpload = async (forcedDoc?: { uri: string, type: string }) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                Alert.alert("ÉXITO", "Documento guardado en tu Bóveda Segura.");
            } else {
                Alert.alert("ERROR", data.error || "Fallo en la subida.");
            }
        } catch (e) {
        ...
        <View style={{ flex: 1, backgroundColor: '#0A0A0A' }}>
            <ScrollView ref={scrollViewRef} style={{ flex: 1 }} contentContainerStyle={{ padding: 20, paddingTop: 60, paddingBottom: 220, flexGrow: 1 }}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                    <Text style={{ color: '#B0B0B0', fontSize: 11, fontWeight: 'bold', letterSpacing: 1.5 }}>🔐 EXPEDIENTES DE ASISTENCIA</Text>
                </View>
                ...
                {documents.filter(d => d.source !== 'DOCS').length === 0 ? (
                    <View style={{ backgroundColor: '#111', padding: 20, borderRadius: 16, marginBottom: 24, alignItems: 'center' }}>
                        <Text style={{ color: '#555', fontSize: 13 }}>No hay expedientes automáticos todavía.</Text>
                    </View>
                ) : (
