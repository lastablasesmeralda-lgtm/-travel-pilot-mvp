...
export default function GlobalOverlays() {
    ...
    React.useEffect(() => {
        ...
        if (isCancelled || isDelayed) {
            if (isCancelled) {
                // RED ALERT: Protocolo de Rescate por Cancelación para TODOS (VIP y Estándar adaptado)
                setShowSOS(false);
                setShowVIPAlternatives(false);
                setShowCancellation(true);
            } else {
                // Protocolo estándar para retrasos
                showPlan();
            }
                                    ...
                                    <View style={{ width: '100%', marginTop: 10 }}>
                                        {(() => {
                                            ...
                                            const renderOption = (opt: any, isMain: boolean) => {
                                                ...
                                                // VIP OVERRIDE para la opción principal si el usuario es VIP
                                                if (isMain && travelProfile === 'premium') {
                                                    borderColor = '#D4AF37'; // Bañado en Oro
                                                    icon = '💎';
                                                    typeLabel = 'ASISTENCIA ELITE';
                                                }
                                                    ...
                                                    <View style={{ backgroundColor: 'rgba(175, 82, 222, 0.05)', padding: 15, borderRadius: 12, marginBottom: 20, borderWidth: 1, borderColor: 'rgba(175, 82, 222, 0.2)' }}>
                                                        ...
                                                        <Text style={{ color: '#FFF', fontSize: 13, lineHeight: 20 }}>
                                                            {flightData?.status?.toLowerCase().includes('cancel') 
                                                              ? `He analizado la cancelación de tu vuelo ${flightData?.flightNumber}. `
                                                              : `He analizado ${flightData?.departure?.delay || 0} min de retraso. `
                                                            }
                                                            Según tu prioridad <Text style={{ color: '#AF52DE', fontWeight: 'bold' }}>{travelProfile === 'premium' ? 'VIP' : travelProfile === 'fast' ? 'Rápida' : travelProfile === 'budget' ? 'Económica' : 'ESTÁNDAR'}</Text>, esta es mi propuesta de rescate.
                                                            Pulsa para ejecutarla.
                                                        </Text>
                                                    </View>
                    ...
                    <SafeAreaView style={[s.mc, { width: '90%', height: '80%', padding: 0, overflow: 'hidden' }]}>
                        ...
                        <View style={{ flex: 1, backgroundColor: '#000', padding: 20 }}>
                            ...
                            <View style={{ marginTop: 40, alignItems: 'center' }}>
                                {(!browserLogs.some((l: string) => l.includes('✅') || l.includes('❌'))) ? (
                                    <>
                        ...
                        <TouchableOpacity style={{ padding: 15, backgroundColor: '#27C93F', alignItems: 'center' }} onPress={() => {
                            ...
                            if (selectedPlan) {
                                ...
                                if (selectedPlan.voiceScriptFinal) {
                                    ...
                                    speak(
                                        isVip
                                            ? 'Todo está listo. He organizado tu plan de asistencia completo en tu sección de Documentos. Tienes todo lo que necesitas para el siguiente paso.'
                                            : isHotel
                                                ? 'He organizado las mejores opciones de descanso para ti. Recuerda que la aerolínea tiene la obligación legal de facilitarte los vales. Encontrarás todos los pasos en tu sección de Documentos.'
                                                : isEco
                                                    ? 'Todo preparado. Tu reclamación legal está lista y organizada en tu sección de Documentos. Puedes presentarla cuando quieras.'
                                                    : 'He terminado de organizar tu plan. Puedes consultar todos los pasos a seguir en tu sección de Documentos.'
                                    );
                                }
