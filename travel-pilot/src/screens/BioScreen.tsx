            ...
            <View style={{ padding: 20 }}>
                ...
                <View style={[s.statsCard, { flexDirection: 'column' }]}>
                    <Text style={{ color: '#B0B0B0', fontSize: 11, marginBottom: 15, lineHeight: 16 }}>
                        La IA utilizará este perfil para tomar decisiones automáticas y buscar la solución óptima en caso de cancelación o retraso grave.
                    </Text>
                    ...
                    <View style={{ gap: 10 }}>
                        ...
                        >
                            ...
                            <Text style={{ color: '#B0B0B0', fontSize: 11 }}>Prioriza opciones baratas o compensaciones altas, sacrificando tiempo si es necesario.</Text>
                        </TouchableOpacity>
                        ...
                        >
                            ...
                            <Text style={{ color: '#B0B0B0', fontSize: 11 }}>Busca el mejor cruce entre coste y tiempo de espera. La opción recomendada por defecto.</Text>
                        </TouchableOpacity>
                        ...
                        >
                            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 4 }}>
                                <Text style={{ fontSize: 18, marginRight: 8 }}>💎</Text>
                                <Text style={{ color: travelProfile === 'premium' ? '#D4AF37' : '#FFF', fontSize: 14, fontWeight: 'bold' }}>MODO VIP {travelProfile !== 'premium' ? '(BLOQUEADO)' : ''}</Text>
                            </View>
                            <Text style={{ color: '#B0B0B0', fontSize: 11 }}>{travelProfile === 'premium' ? 'Protección total activada.' : 'Haz clic para desbloquear el poder total del asistente.'}</Text>
                        </TouchableOpacity>
                    </View>
                </View>
                ...
                <View style={s.statsCard}>
                    <View style={{ flex: 1 }}>
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                            {availableVoices.filter((v: any) => !v.isPremium).slice(0, 4).map((v: any, i: number) => (
                                <TouchableOpacity
                                    key={v.uniqueId || v.identifier}
                                    style={[s.voiceBtn, selectedVoice === v.identifier && s.voiceBtnSelected]}
                                    onPress={() => { setSelectedVoice(v.identifier); speak('Hola, soy tu asistente de viaje inteligente. ¿En qué puedo ayudarte?', v.identifier); }}
                                >
                        ...
                        <Text style={{ color: '#D4AF37', fontSize: 10, marginTop: 16, marginBottom: 6 }}>✨ VOCES PREMIUM EXPERIMENTALES</Text>
                        <View style={{ flexDirection: 'row', gap: 8 }}>
                            {availableVoices.filter((v: any) => v.isPremium).slice(0, 2).map((v: any) => (
                                <TouchableOpacity 
                                    key={v.uniqueId || v.identifier}
                                    onPress={() => { setSelectedVoice(v.identifier); speak('Hola, soy tu asistente de viaje inteligente. ¿En qué puedo ayudarte?', v.identifier); }}
                                    style={{ flex: 1, backgroundColor: selectedVoice === v.identifier ? '#D4AF37' : '#1A1500', borderWidth: 1, borderColor: '#D4AF37', borderRadius: 10, paddingHorizontal: 14, paddingVertical: 8 }}
                                >
                                    <Text style={{ color: selectedVoice === v.identifier ? '#000' : '#D4AF37', fontSize: 11, fontWeight: 'bold' }}>
                                        {v.humanName?.toUpperCase() || 'GEMINI IA'}
                                    </Text>
                                    <Text style={{ color: selectedVoice === v.identifier ? '#333' : '#B0B0B0', fontSize: 9 }}>Castellano · Gemini</Text>
                                </TouchableOpacity>
                            ))}
                        </View>
                        <Text style={{ color: '#555', fontSize: 9, marginTop: 8 }}>Activadas temporalmente de forma gratuita en modo Beta.</Text>
                    </View>
                </View>
            ...
            <Modal visible={showVip} animationType="slide" transparent>
                <VipScreen 
                    ...
                    onActivate={() => {
                        ...
                        setShowVip(false);
                        Alert.alert('💎 ¡BIENVENIDO A ÉLITE!', 'Has activado tu plan de protección total. Tu asistente ahora es un Guardián VIP.');
                    }} 
                />
            </Modal>
        </ScrollView>
